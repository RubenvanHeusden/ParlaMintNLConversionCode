...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:key name="speakerLookup" match="listPerson/person/persName" use="@orig_name"/>
    ...
    <xsl:template match="officiele-publicatie/handelingen/*/spreekbeurt">
        <xsl:element name="u" xmlns="http://www.tei-c.org/ns/1.0">
            ...
            <xsl:attribute name="who">
                <xsl:value-of select = "if(contains(lower-case(spreker/naam/achternaam), 'voorzitter')) then $voorzitter_id else concat(string-join(tokenize(spreker/voorvoegsels, ' ')), string-join(tokenize(spreker/naam/achternaam, ' ')) )"/>
            </xsl:attribute>
            
            <!-- Determine whether the speaker is a chair or a regular (or a guest) -->
            
            <xsl:attribute name="ana">
                <xsl:value-of select="'#' || spreker/@ana "/>
